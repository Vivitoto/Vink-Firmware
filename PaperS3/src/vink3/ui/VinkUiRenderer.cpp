...
void VinkUiRenderer::renderReaderSettings() {
    ...
    const char* pageTurnEngineVal = g_displayService.readerPageTurnEngineLabel();
    ...
    // ══════ 阅读 / 显示：title + 8 rows ══════
    {
        ...
        ry += kRowH; cardDiv(ry);
        cardRow(ry, "翻页效果", pageTurnEngineVal);
        ...
        case SystemState::Settings:
            if (showReaderSettings_) {
                ...
                }
                {
                    ...
                    ry += kRowH;
                    if (inRect(x, yy, kMarginX + 22, ry, kContentW - 44, kRowH)) return UiAction::CycleReaderPageTurnEngine;
                }
                break;
            }
            if (showSystemSettings_) {
