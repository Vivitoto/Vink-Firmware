...
public:
    ...
    // Returns and clears the direction flag set by the last handleTap that triggered a page turn.
    // True = nextPage (forward), false = prevPage (backward) or not a page-turn tap.
    ...
    bool lastTapNextPage_ = false;  // tracks direction of last tap-triggered page turn
