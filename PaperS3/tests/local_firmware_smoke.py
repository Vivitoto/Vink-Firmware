...
def vink3_source_invariants(main_cpp: str) -> None:
    ...
    assert_contains(upstream, "kVinkPaperS3FirmwareVersion = \"v0.3.21\"", "single firmware version constant matches the manifest top version")
    ...
    assert_contains(reader_cpp, "antiAliasEnabled_", "reader text renderer has a formal anti-alias toggle")
    assert_contains(reader_cpp, "梦西游-style AntiAlias switch", "anti-alias follows the EDC Book/Mengxiyou soft-edge toggle model")
    ...
    assert_contains(reader_book_cpp, "VPG4", "reader page cache validates file size/fingerprint/schema and invalidates v0.3.20 page cache")
