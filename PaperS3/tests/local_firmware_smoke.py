...
def vink3_source_invariants(main_cpp: str) -> None:
    ...
    epdiy_backend_h = read("src/vink3/display/EpdiyPaperS3Backend.h")
    epdiy_backend_cpp = read("src/vink3/display/EpdiyPaperS3Backend.cpp")
    epdiy_board_c = read("lib/epdiy/src/board/epd_board_m5papers3.c")
    epdiy_render_c = read("lib/epdiy/src/render.c")
    epdiy_render_ctx_h = read("lib/epdiy/src/output_common/render_context.h")
    epdiy_render_ctx_c = read("lib/epdiy/src/output_common/render_context.c")
    epdiy_render_lcd_c = read("lib/epdiy/src/output_lcd/render_lcd.c")
    epdiy_highlevel_c = read("lib/epdiy/src/highlevel.c")
    epdiy_h = read("lib/epdiy/src/epdiy.h")
    epd_board_h = read("lib/epdiy/src/epd_board.h")
    ...
    assert_contains(epd_board_h, "epd_board_m5papers3", "epdiy exposes a PaperS3 board definition")
    assert_contains(epdiy_board_c, "M5GFX PaperS3 pin map", "epdiy PaperS3 board documents the official M5GFX pin source")
    assert_contains(epdiy_board_c, ".pixel_clock = 16 * 1000 * 1000", "epdiy PaperS3 board starts from the M5GFX-validated 16MHz pclk")
    assert_contains(epdiy_board_c, ".data[0] = M5P_D0", "epdiy PaperS3 board maps the ED047TC1 data bus")
    assert_contains(epdiy_board_c, "epd_lcd_init", "epdiy PaperS3 board uses the LCD/RMT/GDMA renderer")
    assert_contains(epdiy_backend_h, "architecture-level PaperS3 display backend", "Vink has an explicit epdiy architecture backend")
    assert_contains(epdiy_backend_cpp, "releaseBusForExternalDriver", "epdiy backend releases M5GFX Panel_EPD before taking the ED047TC1 bus")
    assert_contains(epdiy_backend_cpp, "EPD_LUT_1K | EPD_FEED_QUEUE_32", "epdiy backend passes an explicit LUT option with the feed queue option")
    assert_not_contains(epdiy_backend_cpp, "EPD_OPTIONS_DEFAULT | EPD_FEED_QUEUE_32", "epdiy backend must not OR zero-valued default options with feed queue flags")
    assert_contains(epdiy_backend_cpp, "epd_init(&epd_board_m5papers3, &ED047TC1", "epdiy backend initializes ED047TC1 through the PaperS3 board")
    assert_contains(epdiy_backend_cpp, "epd_hl_update_area_ex", "epdiy backend routes page turns through epd_hl_update_area_ex")
    assert_contains(epdiy_render_ctx_h, "scroll_conversion_luts", "epdiy render context caches per-phase LUTs for scroll")
    assert_contains(epdiy_render_c, "frames + scroll_count - 1", "epdiy scroll uses one renderer-level phase/progression sequence")
    assert_contains(epdiy_render_c, "waveform_frames > 15 ? 15 : waveform_frames", "epdiy scroll caps ED047TC1 long waveforms to the EDCBook-style 0..15 progression window")
    assert_contains(epdiy_render_c, "(sx & 0x0F) || (ex & 0x0F)", "epdiy scroll rejects buckets that are unsafe for S3 vector LUT alignment")
    assert_contains(epdiy_render_c, "#ifndef RENDER_METHOD_LCD", "epdiy scroll keeps non-LCD render builds on the normal renderer")
    assert_not_contains(epdiy_render_c, "for (int si = 0; si < scroll_count; si++) {\n        int idx = scroll_direction", "epdiy scroll must not run one complete waveform per strip")
    assert_contains(epdiy_render_ctx_c, "phase_time > frame_time", "epdiy scroll frame timing uses the max active bucket-local phase time")
    assert_contains(epdiy_render_ctx_c, "scroll_conversion_luts != NULL", "epdiy scroll does not rebuild the normal LUT when per-phase LUTs are cached")
    assert_contains(epdiy_render_lcd_c, "phase = ctx->current_frame - progression", "epdiy LCD renderer selects bucket-local waveform phases during scroll")
    assert_contains(epdiy_backend_cpp, "logical (x,y) -> physical (y, 539-x)", "epdiy backend has one documented portrait-to-landscape framebuffer boundary")
    assert_contains(display_cpp, "VINK_USE_EPDIY_BACKEND", "DisplayService keeps epdiy backend opt-in until real-device bring-up")
    assert_contains(display_cpp, "pushEpdiyCompositedPageCover", "epdiy page-turn default must use true old-page-stationary new-page-cover semantics, not fixed-position wipe")
    assert_contains(display_cpp, "Do not use the old epdiy fixed-position bucket wipe as the default", "epdiy page-turn must explicitly reject fixed-position wipe as the target effect")
    assert_contains(display_cpp, "composeHorizontalPageCoverFrame", "M5GFX fallback page-cover keeps old page fixed while final-position new page covers it")
    assert_contains(display_cpp, "old page stays fixed", "page-turn compositor must implement EDCBook-like cover-line semantics rather than dual-page sliding")
    assert_contains(display_cpp, "pushPageCoverFrame", "epdiy true page-cover must use direct fused native difference frames, not app-layer RGB565 transition canvases")
    assert_contains(epdiy_backend_cpp, "buildPageCoverDifferenceFrame", "epdiy page-cover builds fused old/new transition differences without highlevel full-frame diff per step")
    assert_contains(epdiy_backend_cpp, "fused page-cover frame update", "epdiy page-cover should bypass per-step front_fb/back_fb memcpy and call the short renderer directly")
    assert_contains(epdiy_highlevel_c, "epd_draw_base_frames", "epdiy highlevel short updates cap the physical waveform frames")
    assert_contains(epdiy_h, "epd_draw_base_frames", "epdiy public render API exposes capped-frame drawing for transition-frame page turns")
    assert_contains(display_cpp, "waveform=private-old-new-lut", "page-turn logs must state the private old/new-aware waveform path")
    assert_contains(display_h, "ReaderPageTurnResidue", "single page-turn residue control is exposed as an active setting")
    assert_contains(display_cpp, "turnresid", "single page-turn residue control persists in display NVS")
    assert_contains(webui_cpp, "pageTurnResidue", "WebUI exposes active single-turn residue compensation")
    assert_contains(ui_cpp, "CycleReaderPageTurnResidue", "device UI can cycle single-turn residue compensation")
    ...
    assert_contains(bus_epd_cpp, "esp_lcd_panel_io_del", "patched Bus_EPD deletes the M5GFX panel IO before epdiy takeover")
    assert_contains(bus_epd_cpp, "esp_lcd_del_i80_bus", "patched Bus_EPD deletes the M5GFX i80 bus before epdiy takeover")
