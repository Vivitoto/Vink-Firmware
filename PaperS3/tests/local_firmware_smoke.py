...
def vink3_source_invariants(main_cpp: str) -> None:
    ...
    assert_contains(cjk_cpp, "primary fallback active", "CJK renderer must log when SPIFFS SC font is not actually the active primary font")
    assert_contains(cjk_cpp, "ReadPaper V3 UI subset glyph fallback armed", "ReadPaper UI subset is armed as a glyph-level fallback for missing UI Chinese")
    ...
    assert_contains(cjk_cpp, "UI callers pass y as the top of a text box", "UI gray font rendering must not baseline-stagger Latin letters")
    assert_contains(cjk_cpp, "ReadPaper glyph yOffset is baseline-oriented", "UI ReadPaper fallback glyphs must not baseline-stagger Latin letters")
    assert_contains(reader_cpp, "visual line-box top", "reader ReadPaper glyph rendering must not baseline-stagger Latin letters")
