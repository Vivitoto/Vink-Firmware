...
def vink3_source_invariants(main_cpp: str) -> None:
    ...
    bus_epd_cpp = read(".pio/libdeps/m5papers3/M5GFX/src/lgfx/v1/platforms/esp32/Bus_EPD.cpp")
    bus_epd_h = read(".pio/libdeps/m5papers3/M5GFX/src/lgfx/v1/platforms/esp32/Bus_EPD.h")
    ...
    ttf_cpp = read("src/vink3/reader/TtfFont.cpp")
    ...
    assert_not_contains(platformio, "VINK_USE_EPDIY_BACKEND", "PlatformIO no longer documents the removed epdiy bring-up flags")
    assert_not_contains(platformio, "lib/epdiy", "PlatformIO no longer references the removed lib/epdiy tree")
    ...
    assert_contains(input_cpp, "kTapSlopPx = 50", "touch tap slop follows EDCBook-style <50px gesture tolerance")
    assert_contains(input_cpp, "kLiftDebounceFrames = 3", "touch release is debounced across multiple missing GT911 frames")
    assert_contains(input_cpp, "kDebounceMs = 18", "touch press-edge debounce stays light for responsive repeated taps")
    assert_contains(input_cpp, "readerQuickTurnZone", "reading body has an eager page-turn touch zone")
    assert_contains(input_cpp, "kReaderQuickTurnStableMs = 35", "reading body can pre-trigger page turns after a short stable press")
    assert_contains(input_cpp, "pendingReaderTurn_", "reading body can preserve one page-turn intent during display push")
    assert_contains(state_cpp, "MessageType::PageNext", "state machine accepts eager reading page-next messages")
    assert_contains(state_cpp, "isReadingBodyVisible", "eager page-turn messages are limited to the reading body")
    ...
    assert_contains(runtime_cpp, "sanitizeBootFlagsForCurrentFirmware", "freshly flashed firmware clears stale NVS boot flags before side-key shutdown handling")
    assert_contains(runtime_cpp, "prefs.getString(\"fw\"", "boot flag guard records firmware version in NVS")
    assert_contains(runtime_cpp, "if (!freshFirmwareBoot)", "first boot of a new RC must not enter stale side-key shutdown fast path")
    assert_contains(runtime_cpp, "putString(\"fw\", kVinkPaperS3FirmwareVersion)", "runtime running/lock flags are version-scoped")
    ...
    assert_contains(ttf_cpp, "fontInfoReady_", "SD TTF keeps parsed stb_truetype fontinfo instead of reinitializing per glyph")
    assert_contains(ttf_cpp, "static uint8_t bmpBuf", "SD TTF draw path reuses a scratch buffer instead of malloc/free per glyph")
    assert_contains(ttf_cpp, "nameBuf", "SD TTF scanner copies entry names before File.close")
    ...
    assert_contains(ui_cpp, "CycleReaderPageTurnEngine", "device settings page can cycle page-turn engine (serial vs continuous)")
    ...
    assert_contains(display_cpp, "actual PaperS3 panel driver is Panel_EPD", "page-turn animation documents the actual PaperS3 panel driver")
    assert_contains(display_cpp, "M5DisplayStripSweep(canvas, effect, mode)", "page-turn animation keeps the historical portrait strip context")
    assert_not_contains(display_cpp, "static_cast<lgfx::Panel_EPDiy*>", "page-turn animation must not reinterpret PaperS3 Panel_EPD as removed alternative panel types")
    assert_not_contains(display_h, "EpdiyHighlevelState", "display service must not expose fake highlevel display state on PaperS3")
    ...
    assert_contains(display_cpp, "const uint16_t stripWidth = pageTurnScrollStripWidth();", "runtime page-turn profile must control actual Panel_EPD scroll width")
    assert_contains(display_cpp, "return 24;  // ~23 passes", "page-turn clean profile uses a thin line-like strip width")
    assert_contains(display_cpp, "return 32;  // ~17 passes", "page-turn balanced profile uses a narrow strip width")
    assert_contains(display_cpp, "return 48;  // ~12 passes", "page-turn fast profile is capped to a narrow v0.4.28-like strip width")
    assert_contains(display_cpp, "turnprof", "page-turn profile persists in display NVS to reduce reflashes during tuning")
    assert_contains(display_cpp, "known-good v0.4.28 logical-strip", "normal M5GFX page turns intentionally restore the known EDCBook-flavor logical-strip behavior")
    assert_not_contains(display_cpp, "? kLowRefresh", "reader page turns must not convert text pushes to epd_fastest")
    assert_contains(display_cpp, "smooth text waveform", "normal page turns use the stable standard-LUT/text baseline")
    ...
    assert_contains(display_cpp, "known-good v0.4.28 logical-strip", "normal M5GFX page turns use the known EDCBook-flavor logical-strip path")
    assert_contains(display_cpp, "pushEdcBookPageTurn(canvasToPush", "normal M5GFX page turns enter Panel_EPD wavefront directly, not the full-page cover compositor")
    assert_contains(display_cpp, "speed is decoupled and controlled by ReaderPageTurnProfile strip width", "full-clean frequency is decoupled from page-turn animation")
    assert_not_contains(display_cpp, "VINK_USE_EPDIY_BACKEND", "DisplayService no longer references the removed epdiy backend")
    assert_not_contains(display_cpp, "g_epdiyPaperS3Backend", "DisplayService no longer calls into the removed epdiy backend")
    assert_not_contains(display_cpp, "pushEpdiyCompositedPageCover", "DisplayService no longer carries the removed epdiy page-cover compositor")
    assert_not_contains(display_cpp, "EpdiyPaperS3Backend.h", "DisplayService no longer includes the removed epdiy backend header")
    assert_not_contains(runtime_cpp, "EpdiyPaperS3Backend", "VinkRuntime no longer references the removed epdiy backend")
    assert_not_contains(runtime_cpp, "VINK_USE_EPDIY_BACKEND", "VinkRuntime no longer references the removed epdiy bring-up flag")
    assert_contains(display_cpp, "normalMode = kNormalRefresh", "reader page-turn default mode must be the smooth text waveform, not epd_fastest dot-matrix Bayer")
    assert_contains(display_cpp, "thresholds every pixel through a 4x4 Bayer", "DisplayService documents why epd_fastest cannot be the reader default")
    assert_contains(display_cpp, "return kMiddleRefresh;", "non-reader UI uses the faster middle waveform for responsiveness")
    assert_contains(display_cpp, "const epd_mode_t sweepMode = kNormalRefresh", "page-turn scroll forces the smooth text waveform; epd_fast/epd_fastest never reach the reveal")
    assert_contains(display_cpp, "v0.4.28 edcscroll Panel_EPD logical-strip", "page-turn logs must state the restored v0.4.28 logical-strip path")
    assert_contains(display_cpp, "panel->displayScroll(0, 0, kPaperS3Width, kPaperS3Height, stripWidth, rtl,", "normal no-epdiy page turns pass only the engine selector (no private waveform/residue parameters)")
    assert_contains(display_cpp, "turnresid", "legacy single page-turn residue NVS can still be read without affecting the active path")
    assert_not_contains(webui_cpp, "pageTurnResidue", "WebUI must not expose disabled private waveform/residue tuning")
    assert_not_contains(ui_cpp, "CycleReaderPageTurnResidue", "device UI must not expose disabled private waveform/residue tuning")
    assert_not_contains(display_cpp, "ReaderGhostingProfile", "obsolete ghost-compensation experiments must not remain as user-visible settings")
    assert_not_contains(display_h, "ReaderGhostingProfile", "display header must not expose obsolete ghost-compensation tuning")
    assert_not_contains(webui_cpp, "ghostingProfile", "WebUI must not expose obsolete ghost-compensation tuning")
    assert_not_contains(ui_cpp, "CycleReaderGhostingProfile", "device UI must not expose obsolete ghost-compensation tuning")
    ...
    assert_contains(panel_epd_h, "releaseBusForExternalDriver", "patched Panel_EPD exposes a controlled external-driver release hook")
    assert_contains(bus_epd_h, "void release(void) override", "patched Bus_EPD exposes esp_lcd bus release")
    assert_contains(bus_epd_cpp, "esp_lcd_panel_io_del", "patched Bus_EPD deletes the M5GFX panel IO before any future external takeover")
    assert_contains(bus_epd_cpp, "esp_lcd_del_i80_bus", "patched Bus_EPD deletes the M5GFX i80 bus before any future external takeover")
    assert_contains(panel_epd_h, "scroll_waveform", "Panel_EPD update data can request the private page-turn waveform")
    assert_contains(panel_epd_cpp, "lut_page_turn", "Panel_EPD retains the private short page-turn LUT only as an optional experiment")
    assert_contains(panel_epd_cpp, "preserves anti-aliased", "private page-turn LUT remains documented for future audit")
    ...
    assert_contains(panel_epd_cpp, "flg_page_turn", "Panel_EPD scroll worker can still use the private page-turn waveform when explicitly enabled")
    assert_contains(panel_epd_cpp, "upd.scroll_waveform = false", "normal displayScroll disables the private waveform by default")
    assert_contains(panel_epd_cpp, "v0.4.59 dual-engine", "Panel_EPD logical-strip scheduler exposes both serial-drain and continuous-flow engines")
    assert_contains(panel_epd_cpp, "v0.4.58 serial-drain engine", "Panel_EPD serial-drain engine documented and still available")
    assert_contains(panel_epd_cpp, "Known-good v0.4.28 behavior", "Panel_EPD has a v0.4.28 logical-strip fallback before rotated transition experiments")
    ...
    assert_contains(panel_epd_cpp, "scroll_offsets", "Panel_EPD scroll uses an EDCBook-like offset progression table")
    assert_contains(panel_epd_cpp, "strip_px = std::max<uint16_t>(4u", "Panel_EPD offset table preserves a minimum 4-pixel step")
    assert_contains(panel_epd_cpp, "transition=(new<<4)|old", "Panel_EPD scroll documents the EDCBook transition-byte model")
    assert_contains(panel_epd_cpp, "transition_frame_t", "Panel_EPD scroll materializes an EDCBook-style TransitionFrame")
    assert_contains(panel_epd_cpp, "dirty_lines", "Panel_EPD scroll tracks EDCBook-style dirty lines")
    assert_contains(panel_epd_cpp, "dirty_columns", "Panel_EPD scroll tracks EDCBook-style dirty columns")
    assert_contains(panel_epd_cpp, "populate_line_mask", "Panel_EPD scroll builds EDCBook-style line masks")
    assert_contains(panel_epd_cpp, "build_edcbook_phase_luts", "Panel_EPD scroll builds EDCBook-style 64-byte waveform phase tables")
    assert_contains(panel_epd_cpp, "build_conversion_lut", "Panel_EPD scroll uses transition-byte conversion LUT lookup")
    assert_contains(panel_epd_cpp, "conversion_lut_slots", "Panel_EPD scroll stores per-bucket conversion LUT slots")
    assert_contains(panel_epd_cpp, "prepare_edcbook_scroll_context_frame", "Panel_EPD scroll prepares per-frame EDCBook-style scroll context")
    assert_contains(panel_epd_cpp, "scroll_bucket_cache", "Panel_EPD scroll caches per-bucket phase/LUT slots like EDCBook")
    assert_contains(panel_epd_cpp, "scroll_line_mask_blocks", "Panel_EPD scroll tracks EDCBook ctx+0x1a8 block gating")
    assert_contains(panel_epd_cpp, "return 0;", "Panel_EPD recovery build disables diagonal row-source offsets for a straight wavefront")
    assert_contains(panel_epd_cpp, "source_x = source_bucket", "Panel_EPD scroll keeps bucket source mapping explicit for future row-offset experiments")
    assert_contains(panel_epd_cpp, "abs_pair = (upd.x >> 1) + pi", "Panel_EPD scroll packs DMA output at the physical x offset")
    assert_contains(panel_epd_cpp, "any_dirty", "Panel_EPD scroll tracks empty-diff early return")
    assert_contains(panel_epd_cpp, "render_x0", "Panel_EPD scroll crops output to the aligned dirty rect")
    assert_contains(panel_epd_cpp, "run_edcbook_transition_frame", "Panel_EPD scroll advances an EDCBook-style transition wavefront inside the scan-cycle worker")
    assert_contains(panel_epd_cpp, "build_edcbook_row_offset", "Panel_EPD scroll adapts EDCBook's per-scanline row_offset_table behavior")
    assert_contains(panel_epd_cpp, "ctx+0x148", "Panel_EPD scroll documents the EDCBook scroll_bucket_cache evidence")
    assert_contains(panel_epd_cpp, "ctx+0x1a8", "Panel_EPD scroll documents the EDCBook scroll_line_mask_blocks evidence")
    assert_contains(panel_epd_cpp, "ctx+0x1b4", "Panel_EPD scroll documents the EDCBook row_offset_table source-remap evidence")
    ...
    assert_contains(state_cpp, "cycleReaderPageTurnEngine", "state machine cycles the dual page-turn engine selector")
