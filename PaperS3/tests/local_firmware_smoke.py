...
def vink3_source_invariants(main_cpp: str) -> None:
    ...
    assert_contains(display_cpp, "ReaderPageTurnProfile::Clean", "page-turn profile has a clean 8px runtime-selectable strip width")
    assert_contains(display_cpp, "ReaderPageTurnProfile::Balanced", "page-turn profile has a balanced 12px runtime-selectable strip width")
    assert_contains(display_cpp, "ReaderPageTurnProfile::Fast", "page-turn profile has a fast 16px runtime-selectable strip width")
    ...
    assert_contains(display_cpp, "private\n    // short page-turn LUT", "normal page turns use Panel_EPD's private short page-turn LUT")
    assert_contains(display_cpp, "pageTurnScrollMode", "page-turn cleanup is integrated into the strip waveform")
    assert_contains(display_cpp, "scheduledMode == kQualityRefresh", "page-turn animation honors full-clean intervals inside the scroll waveform")
    ...
    assert_contains(display_cpp, "rather than\n    // appending a separate flash after the animation", "page-turn full-clean interval must not become an after-animation flash")
    ...
    assert_contains(panel_epd_cpp, "selective light-tail cleanup", "page-turn LUT uses a white-background tail cleanup without fading black text")
