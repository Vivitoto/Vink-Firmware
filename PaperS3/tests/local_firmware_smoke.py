...
def vink3_source_invariants(main_cpp: str) -> None:
    ...
    assert_contains(ui_cpp, "drawSmallText", "shell card notes use smaller secondary text")
    assert_contains(ui_cpp, "drawSmallCentered", "shell footer hints use smaller secondary text")
    ...
    assert_contains(reader_cpp, "Button labels are primary actions", "reader action buttons keep primary labels centered in button geometry")
    assert_contains(reader_cpp, "Summary/help text is secondary", "reader summary/help text uses smaller secondary font")
