...
def vink3_source_invariants(main_cpp: str) -> None:
    ...
    assert_contains(upstream, "kVinkPaperS3FirmwareVersion = \"v0.3.17\"", "single firmware version constant matches the manifest top version")
    ...
    assert_not_contains(state_cpp, "pulsePaperS3PowerOffPin", "shutdown path must not manually pulse PaperS3 PMIC power-off pin")
    assert_not_contains(state_cpp, "M5.Power.powerOff();", "shutdown path must bypass M5.Power.powerOff because real PaperS3 reboots")
    assert_contains(state_cpp, "esp_sleep_disable_wakeup_source(ESP_SLEEP_WAKEUP_ALL)", "shutdown path disables wake sources before deep sleep")
    ...
    assert_contains(reader_book_cpp, "library browser", "reader book service uses file-browser mode for /books")
