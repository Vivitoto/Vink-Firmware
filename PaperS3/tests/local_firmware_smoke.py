...
APP_SLOT_SIZE = 0xC00000  # v0.3 single-app layout for full Vink PROGMEM font
...
def source_invariants() -> None:
    ...
    assert_contains(platformio, "bblanchon/ArduinoJson", "active config/WebUI JSON dependency is listed")
...
def vink3_source_invariants(main_cpp: str) -> None:
    ...
    full_font_h = read("src/vink3/text/VinkBookFont.h")
    gbk_table_h = read("src/vink3/text/GbkUnicodeTable.h")
    upstream = read("src/vink3/VinkPaperS3Core.h")
    platformio = read("platformio.ini")

    assert_contains(main_cpp, "xTaskCreatePinnedToCore", "v0.3 main starts a Vink service-style pinned MainTask")
    assert_contains(upstream, "kVinkPaperS3FirmwareVersion = \"v0.4.3\"", "single firmware version constant matches the manifest top version")
    ...
    assert_contains(runtime_cpp, "kVinkPaperS3CoreReferenceVersion", "v0.3 runtime records Vink upstream baseline")
    remote_sync_name = "Le" + "gado"
    assert_not_contains(runtime_cpp, remote_sync_name, "runtime no longer starts or configures remote reading sync")
    ...
    assert_contains(upstream, "V1.7.6", "v0.3 baseline is Vink reference core")
    assert_contains(upstream, "e910d29", "v0.3 baseline records latest remote commit")
    assert_contains(display_h, "DisplayRequest", "v0.3 display queue has Vink service-style request struct")
    ...
    assert_not_contains(state_cpp, remote_sync_name, "state machine no longer exposes remote reading sync states")
    assert_not_contains(reader_book_cpp, remote_sync_name, "reader progress is local-only after remote sync removal")
    ...
    assert_contains(cjk_cpp, "Vink font v3 UI subset last-resort fallback armed", "Vink UI fallback is only a last-resort fallback for missing UI Chinese")
    ...
    assert_contains(reader_cpp, "Vink font v3 glyph entries already store a visual yOffset", "reader Vink glyph rendering must not center punctuation/lowercase in prompts")
    ...
    assert_contains(reader_cpp, "beginVinkBookFont", "reader body renderer uses full Vink PROGMEM font")
    assert_contains(reader_cpp, "第 %u 页", "reader footer shows streaming page number when total pages are unknown")
    assert_contains(reader_cpp, "Thin visual progress rail", "reader footer keeps progress rail for known-total list/action pages")
    assert_contains(reader_cpp, "fillW", "reader progress rail fills according to percentage when total pages are known")
    ...
    assert_contains(reader_book_cpp, "正在准备当前页", "reader explains streaming current-page preparation")
    assert_contains(reader_book_cpp, "读/目/旧", "library explains progress/TOC/legacy-page-cache status markers")
    ...
    assert_contains(reader_book_cpp, "buildChapterPages", "reader book service prepares streaming chapter pages")
    assert_contains(reader_book_cpp, "appendNextStreamingPage", "reader advances by measuring one page at a time")
    assert_contains(reader_book_cpp, "streaming page ready", "reader logs low-latency streaming page preparation")
    assert_contains(reader_book_cpp, ".vink-progress", "reader book service stores progress beside the source book")
    assert_contains(reader_book_cpp, "VPR3-compatible: chapter-level restore", "reader progress is chapter-level and layout-safe")
    assert_contains(reader_cpp, "第 %u 页", "reader footer supports unknown total pages for streaming pagination")
    assert_not_contains(reader_book_cpp, "saveChapterPageCache", "reader no longer writes persistent page caches")
    assert_not_contains(reader_book_cpp, "loadChapterPageCache", "reader no longer blocks on persistent page caches")
    assert_not_contains(reader_book_cpp, "VPG3", "reader no longer depends on persistent page-cache schema")
    ...
    assert_contains(reader_book_cpp, "pageWindowTruncated_", "reader tracks whether more text remains for streaming pagination")
    ...
    assert_contains(full_font_h, "readpaperFullByte", "v0.3 full Vink font is exposed through the Vink book-font adapter")
    assert_contains(partitions_csv, "0xC00000", "v0.3 partition table has a large single app slot for full Vink font")
    assert_not_contains(partitions_csv, "app1", "v0.3 partition table drops dual OTA app1 to fit full Vink font")
    assert_contains(gbk_table_h, "gbkToUnicode", "v0.3 includes full Vink-owned GBK Unicode lookup")
    ...
    assert_contains(webui_cpp, "apiLibraryGet", "Web UI exposes local TXT library status")
    assert_contains(webui_cpp, "apiExportConfig", "Web UI can export full local config backup")
    assert_contains(ui_cpp, "浏览器打开 http://", "device transfer page shows the Web UI URL hint")
    assert_contains(ui_cpp, "连接 WiFi/热点", "device WiFi page shows connection guidance")
    assert_not_contains(webui_cpp, "apiExportProgress", "Web UI no longer exposes progress export")
    assert_not_contains(webui_cpp, "apiExportLibrary", "Web UI no longer exposes bookshelf export")
    assert_not_contains(webui_cpp, "/screenshots/", "Web UI no longer exposes screenshot directory shortcuts")
    assert_contains(webui_cpp, "apiLibraryCleanupCache", "Web UI can clean generated book caches without deleting progress")
    assert_contains(webui_cpp, "apiLibraryCleanupAllCache", "Web UI can batch-clean generated book caches")
    assert_contains(webui_cpp, "apiLibraryCleanupOrphanCache", "Web UI can clean orphan generated book caches")
    assert_contains(webui_cpp, "cleanupOrphanBookCaches", "Web UI exposes orphan cache cleanup action")
    assert_contains(webui_cpp, "uploadToBooks", "Web UI can upload selected files directly to /books")
    assert_contains(webui_cpp, "panelBooks", "Web UI has a local bookshelf tab")
    assert_contains(webui_cpp, "panelBackup", "Web UI has backup/export actions")
    ...
    assert_not_contains(webui_cpp, remote_sync_name.lower(), "Web UI config no longer exposes remote sync fields")
    assert_contains(wifi_cpp, "max_uri_handlers = 40", "HTTP server allows enough route slots for expanded Web UI handlers")
