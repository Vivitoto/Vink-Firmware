...
def vink3_source_invariants(main_cpp: str) -> None:
    ...
    assert_contains(display_cpp, "ghost=disabled-standard-lut", "page-turn logs must state ghost compensation is disabled on the standard-LUT baseline")
    assert_not_contains(display_cpp, "ReaderGhostingProfile", "disabled ghost-compensation experiments must not remain as user-visible settings")
    assert_not_contains(display_h, "ReaderGhostingProfile", "display header must not expose inactive ghost-compensation tuning")
    assert_not_contains(webui_cpp, "ghostingProfile", "WebUI must not expose inactive ghost-compensation tuning")
    assert_not_contains(ui_cpp, "CycleReaderGhostingProfile", "device UI must not expose inactive ghost-compensation tuning")
    ...
    assert_contains(panel_epd_cpp, "step_px = std::max<uint16_t>(4u", "Panel_EPD expanding-window scroll preserves a minimum 4-pixel step")
    assert_contains(panel_epd_cpp, "((xe + me->_cfg.offset_x) | 3u) - win.x + 1", "Panel_EPD scroll windows keep packed-byte loops 4-pixel aligned")
