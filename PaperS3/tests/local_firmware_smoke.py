...
def vink3_source_invariants(main_cpp: str) -> None:
    ...
    assert_contains(upstream, "kVinkPaperS3FirmwareVersion = \"v0.4.4\"", "single firmware version constant matches the manifest top version")
    ...
    assert_contains(reader_book_cpp, "刷新策略", "reader menu exposes refresh strategy setting")
    ...
    assert_contains(reader_book_cpp, "\"继续阅读\", \"刷新策略\", \"抗锯齿\", \"排版优化\", \"下划线\", \"翻页效果\"", "reader menu exposes all active reader settings")
    ...
    assert_contains(reader_book_cpp, "v0.4.4-on-0.4.2 intentionally ignores persistent .vink-pages caches", "reader ignores stale persistent page caches in streaming mode")
