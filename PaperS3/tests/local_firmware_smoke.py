...
def vink3_source_invariants(main_cpp: str) -> None:
    ...
    assert_contains(upstream, "kVinkPaperS3FirmwareVersion = \"v0.3.9-rc\"", "single firmware version constant matches the manifest top version")
    ...
    assert_contains(reader_book_cpp, "VCT2", "reader TOC cache invalidates when source size or detector version changes")
    ...
    assert_contains(webui_cpp, "while (tail.startsWith(\"//\"))", "Web UI normalizes encoded absolute upload/delete paths")
