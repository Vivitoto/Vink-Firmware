...
def vink3_source_invariants(main_cpp: str) -> None:
    ...
    assert_contains(upstream, "kVinkPaperS3FirmwareVersion = \"v0.4.6\"", "single firmware version constant matches the manifest top version")
    ...
    assert_contains(cjk_cpp, "PROGMEM SC 24px UI font loaded", "compiled UI font is the only normal UI font dependency")
    assert_contains(cjk_cpp, "compiled UI font unavailable", "CJK renderer logs if the compiled UI font is unavailable")
    ...
    assert_contains(reader_book_cpp, "g_uiRenderer.renderUiActionPage(SystemState::Reader, \"阅读菜单\"", "reader menu uses UI font renderer, not body reading font")
    assert_not_contains(reader_book_cpp, "g_readerText.renderActionPage(\"阅读菜单\"", "reader menu must not use body reading font")
    assert_contains(reader_book_cpp, "翻页刷新", "reader menu exposes refresh strategy setting")
    assert_contains(reader_book_cpp, "抗锯齿", "reader menu exposes anti-alias setting")
    assert_contains(reader_book_cpp, "排版优化", "reader menu exposes book layout optimization setting")
    ...
    assert_contains(reader_book_cpp, "\"继续阅读\", \"翻页刷新\", \"抗锯齿\", \"排版优化\", \"下划线\", \"翻页动画\"", "reader menu exposes all active reader settings")
    ...
    assert_contains(reader_cpp, "Vink-native AntiAlias switch", "anti-alias follows the Vink soft-edge toggle model")
    ...
    assert_contains(display_cpp, "epd_mode_t::epd_fastest", "page-turn animation uses IT8951 native DU4 sweep")
    assert_contains(display_cpp, "setRotation(effect == DisplayEffect::VerticalShutter ? 0 : 1)", "page-turn sweep direction flips via IT8951 scan-direction bit")
    assert_contains(display_cpp, "savedRotation", "page-turn restores original rotation after sweep")
    assert_not_contains(display_cpp, "kStripW", "page-turn animation no longer uses software strip rendering")
    assert_not_contains(display_cpp, "copyStrip", "page-turn animation no longer copies per-strip pixel data")
    ...
    assert_contains(reader_book_cpp, "VCT4", "reader TOC cache validates file size, fingerprint, and schema")
    ...
    assert_contains(reader_book_cpp, "clearPageCache", "reader keeps manual legacy page-cache cleanup action")
    ...
    assert_contains(chapter_cpp, "readBytesUntil('\\n'", "chapter detector reads lines in buffered chunks instead of byte-by-byte SD reads")
    assert_contains(chapter_cpp, "looksLikeCandidateLine", "chapter detector cheaply rejects normal prose before expensive matchers")
    assert_contains(chapter_cpp, "Discard overlong physical lines in buffered chunks", "chapter detector discards long non-heading lines in buffered chunks")
    assert_contains(chapter_cpp, "(++linesScanned & 0x3F) == 0", "chapter detector yields periodically during large-book scans")
