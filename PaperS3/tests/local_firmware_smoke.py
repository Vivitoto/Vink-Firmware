...
def vink3_source_invariants(main_cpp: str) -> None:
    ...
    assert_contains(upstream, "kVinkPaperS3FirmwareVersion = \"v0.4.8\"", "single firmware version constant matches the manifest top version")
    ...
    assert_contains(ui_cpp, "drawButton(296, 350, 192, kButtonMinH, \"书架\"", "reader home bookshelf button geometry is explicit")
    ...
    assert_contains(ui_cpp, ""抗锯齿", "翻页刷新", "下划线", "排版优化", "翻页动画", "页边距"", "reader menu exposes all active reader settings in two-column overlay")
    ...
    assert_contains(display_cpp, "kStripW", "page-turn animation uses software strip rendering")
