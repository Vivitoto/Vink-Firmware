...
void lcd_do_update(RenderContext_t* ctx) {
    epd_set_mode(1);
    lcd_do_update_frames(ctx);
    epd_set_mode(0);
}

// Internal: run only the frame loop without toggling EPD mode.
// Used by epd_draw_base_scroll to chain multiple strips into one sweep.
void lcd_do_update_frames(RenderContext_t* ctx) {
