...
// EDCBook-style scroll update: like epd_hl_update_area but uses
// epd_draw_base_scroll for directional strip-by-strip progression.
enum EpdDrawError epd_hl_update_area_ex(
    EpdiyHighlevelState* state, enum EpdDrawMode mode, int temperature,
    EpdRect area, int* scroll_offsets, int scroll_count, int scroll_direction
) {
    assert(state != NULL);
    EpdRect rotated = _inverse_rotated_area(area.x, area.y, area.width, area.height);
    area.x = rotated.x; area.y = rotated.y;
    area.width = rotated.width; area.height = rotated.height;

    EpdRect diff_area = epd_difference_image_cropped(
        state->front_fb, state->back_fb, area,
        state->difference_fb, state->dirty_lines, state->dirty_columns
    );
    if (diff_area.height == 0 || diff_area.width == 0)
        return EPD_DRAW_SUCCESS;

    // Use scroll renderer instead of standard epd_draw_base
    enum EpdDrawError err = epd_draw_base_scroll(
        epd_full_screen(), state->difference_fb, diff_area,
        MODE_PACKING_1PPB_DIFFERENCE | mode, temperature,
        state->dirty_lines, state->dirty_columns, state->waveform,
    ...
    // Keep the high-level front/back model coherent after a successful scroll.
    // EDCBook's scroll path starts from a front/back diff; leaving back_fb stale
    // would make the next page turn compare against an older page and overdrive
    // pixels unrelated to the current transition.
    const int buf_width = epd_width();
    for (int l = diff_area.y; l < diff_area.y + diff_area.height; l++) {
        if (state->dirty_lines[l] > 0) {
            uint8_t* lfb = state->front_fb + buf_width / 2 * l;
            uint8_t* lbb = state->back_fb + buf_width / 2 * l;
